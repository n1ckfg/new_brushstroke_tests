...
def extract_contours(points, iso):
    returns = []
    for contour in measure.find_contours(points, iso):
        contour_points = []
        for point in contour:
            vertex = points[int(point[0])]
            contour_points.append(vertex)
        returns.append(contour_points)
    return returns

strokes = extract_contours(mesh.vertex_matrix(), 0.01) 
